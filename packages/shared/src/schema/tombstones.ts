 ...
 * short_name 墓碑。
 *
 * 个人页被删除后 short_name 迁到这里并**永不释放**：他印在名片上、挂在社媒
 * 简介里的旧链接会返回 404，而不是把访客导到另一个陌生人的页面上。
 * 这就是不做「删除后释放地址」的原因 —— 抢注一个刚被释放的旧地址，
 * 等于白捡别人的流量。
 *
 * 删除一个账号时，其名下**全部**个人页的 short_name 都进墓碑。
 */
export const shortNameTombstones = pgTable('short_name_tombstones', {
  /** 已经是小写，与 profiles.short_name 同一口径 */
  shortName: text().primaryKey(),
  /** 原个人页的 id，仅供排查，该行已经删了，因此不设外键 */
  formerProfileId: uuid().notNull(),
